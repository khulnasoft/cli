...
const { KHULNSOFT_PATH } = require('../../interface/cli/defaults');

const dirPath = path.resolve(KHULNSOFT_PATH, 'cli-config');
...
function _loadFullConfig() {
    if (!fs.existsSync(KHULNSOFT_PATH)) {
        fs.mkdirSync(KHULNSOFT_PATH);
    }
...
class CliConfigManager {
    ...
    static loadConfig() {
        try {
            ...
            console.warn('--------------');
            console.warn('CLI config not loaded -- running in fallback mode.');
            console.warn('--------------\n');

            FULL_CONFIG = FALLBACK_CONFIG;
            CURRENT_CONFIG = Model.default();
        }
    }
