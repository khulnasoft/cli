    ...
    return async (argv) => {
        try {
            ...
            if (requiresAuthenticationValue && (!sdk.config || !sdk.config.context || sdk.config.context.isNoAuth)) {
                Output.printError(new CFError('Authentication error: Please create an authentication context (see khulnasoft auth create-context --help)'));// eslint-disable-line
                process.exit(1);
            }
