...
const debug = require('debug')('khulnasoft:cli:helpers:auth');
...
const printTableForAuthContexts = ({ filter = 'all' } = {}) => {
    ...
    if (!currentContext && (!sdkContext || sdkContext.isNoAuth)) {
        console.log('No authentication contexts. Please create an authentication context (see khulnasoft auth create-context --help)');
