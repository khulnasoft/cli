...
const command = new Command({
    ...
    builder: (yargs) => {
        return yargs
            ...
            .example('khulnasoft get build ID', 'Get build ID')
            .example('khulnasoft get builds', 'Get all builds')
            .example('khulnasoft get builds --pipeline-id ID', 'Get all builds that are executions of pipeline "ID"')
            .example('khulnasoft get builds --pipeline-name NAME', 'Get all builds that are executions of pipelines that are named "NAME"')
            .example('khulnasoft get builds --status=error', 'Get all builds that their status is error')
            .example('khulnasoft get builds --from=2020-01-01 --to=2020-02-01', 'Get all builds within given date range');
    },
    handler: async (argv) => {
