...
const { parseFamiliarName } = require('@khulnasoft/docker-reference');
...
const command = new Command({
    ...
    builder: (yargs) => {
        return yargs
            ...
            .example('khulnasoft annotate image 2dfacdaad466 -l coverage=75%', 'Annotate an image with a single label')
            .example('khulnasoft annotate image 2dfacdaad466 -l coverage=75% -l tests_passed=true', 'Annotate an image with multiple labels');
    },
    handler: async (argv) => {
