...
const command = new Command({
    ...
    aliases: ['repo'],
    parent: addRoot,
    description: 'Add a repository from git context to khulnasoft',
    webDocs: {
        category: 'Repository',
    ...
    builder: (yargs) => {
        yargs
            ...
            .example('khulnasoft add repo khulnasoft/some-repo', 'Add repo "some-repo" of "khulnasoft" owner from default git context')
            .example('khulnasoft add repo khulnasoft/some-repo -c bitbucket', 'Add repo "some-repo" of "khulnasoft" owner from git context "bitbucket"');
        return yargs;
    },
    handler: async (argv) => {
        ...
        if (!fullname) {
            throw new CFError('Full repo name is not provided');
        }

        const [owner, repoName] = fullname.split('/');
        if (!owner) {
            throw new CFError('Owner name not provided! Please follow the repo name template: "<owner>/<repo>"');
        }
        if (!repoName) {
            throw new CFError('Repo name not provided! Please follow the repo name template: "<owner>/<repo>"');
        }

        try {
            // throws on not found
            const gitRepo = await sdk.repos.git.get({ owner, repo: repoName, context });
            ...
            try {
                ...
                fullname = `${contextPrefix}/${fullname}`; // aka github-github/khulnasoft/cf-api
            } catch (e) {
