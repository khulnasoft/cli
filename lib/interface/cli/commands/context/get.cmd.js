...
const command = new Command({
    ...
    builder: (yargs) => {
        return yargs
            ...
            .example('khulnasoft get context NAME', 'Get context NAME')
            .example('khulnasoft get contexts', 'Get all contexts')
            .example('khulnasoft get context --decrypt', 'Get all contexts with credentials decrypted')
            .example('khulnasoft get context --type secret', 'Get all secret contexts')
            .example('khulnasoft get context --type git.github', 'Get all git based contexts for github kind')
            .example('khulnasoft get context --type helm-repository', 'Get all helm-repository contexts');
    },
    handler: async (argv) => {
        const data = {};
