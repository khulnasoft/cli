require('debug')('khulnasoft:cli:create:trigger-event');
...
const command = new Command({
    ...
    builder: (yargs) => {
        yargs
            ...
            .example('khulnasoft create trigger-event --type registry --kind dockerhub --value namespace=khulnasoft --value name=fortune --value action=push', 'Create registry/dockerhub trigger-event')
            .example('khulnasoft create trigger-event --type cron --kind khulnasoft --value expression="0 0 */1 * * *" --value message=hello', 'Create cron (once in hour) trigger-event')
            .example('khulnasoft create trigger-event --type cron --kind khulnasoft --value expression="@daily" --value message=hello-all', 'Create daily cron trigger-event');
