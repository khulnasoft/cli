...
async function createAgent(argv) {
    ...
    try {
        ...
        console.log(`A Khulnasoft Runner with the name: ${colors.cyan(finalName)} has been created.`);
        return token;
    } catch (err) {
        const msg = Output._extractErrorMessage(err);
        if (msg.includes('Agent name duplication')) {
            throw new Error(`A Khulnasoft Runner with the name "${colors.cyan(finalName)}" already exists. Please choose a different name, or delete the current agent from the platform.`);
        }
...
async function installAgentInCluster(argv, token, agentName) {
    ...
    let totalSize;
    events.onStart((size) => {
        console.log('Downloading Khulnasoft Runner installer \n');
