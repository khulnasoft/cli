...
async function promptConfirmationMessage({
    ...
    console.log(`\u2022 Khulnasoft runner with the name "${colors.cyan(agentName)}"`);
    attachedRuntimes.forEach((reName) => { console.log(`\u2022 Khulnasoft runtime with the name "${colors.cyan(reName)}"${defaultRuntime === reName ? ' [default runtime]' : ''}`); });
    console.log('\u2022 Khulnasoft runner monitor component');
    clustersToDelete.forEach((cluster) => { console.log(`\u2022 Khulnasoft cluster integration: "${colors.cyan(cluster.selector)}"`); });
...
const deleteCmd = new Command({
    ...
    parent: runnerRoot,
    command: 'delete',
    description: 'Deletes khulnasoft runner solution\'s components on kubernetes cluster',
    webDocs: {
        category: 'Runner',
        title: 'Delete',
        weight: 100,
    ...
    builder: yargs => yargs
        ...
        .option('keep-cluster-integration', {
            describe: 'If true, will not delete dangling cluster integrations from Khulnasoft',
            type: Boolean,
        })
        .option('values', {
    ...
    handler: async (argv) => {
        ...
        if (!agents || !agents.length) {
            console.log('No runners found on your khulnasoft account');
            process.exit(0);
        }
        ...
        if (!kubeNamespace) {
            ...
            if (!relatedNamespaces.length) {
                answer = await inquirer.prompt({
                    ...
                    message: 'Kubernetes namespace to remove Khulnasoft Runner components from',
...
`);
        ...
        await Promise.all(attachedRuntimes.map(async (reName) => {
            ...
            // delete khulnasoft entity
            console.log(`Deleting runtime-environment: "${colors.cyan(reName)}" from khulnasoft`);
        ...
        console.log('Successfully uninstalled Khulnasoft Runner');
        console.log(`\nIf you had any issues with the uninstallation process please report them at: ${colors.blue('https://github.com/khulnasoft/cli/issues/new')}`);
