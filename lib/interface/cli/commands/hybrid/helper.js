...
const INSTALLATION_DEFAULTS = {
    ...
    RUNTIME_SERVICE_ACCOUNT: 'khulnasoft-engine',
...
const RUNTIME_IMAGES = {
    DOCKER_PUSHER_IMAGE: 'khulnasoft/cf-docker-pusher:6.0.3',
    DOCKER_PULLER_IMAGE: 'khulnasoft/cf-docker-puller:8.0.2',
    DOCKER_TAG_PUSHER_IMAGE: 'khulnasoft/cf-docker-tag-pusher:1.3.0',
    DOCKER_BUILDER_IMAGE: 'khulnasoft/cf-docker-builder:1.1.12',
    GC_BUILDER_IMAGE: 'khulnasoft/cf-gc-builder:0.4.4',
    CONTAINER_LOGGER_IMAGE: 'khulnasoft/cf-container-logger:1.6.1',
    GIT_CLONE_IMAGE: 'khulnasoft/cf-git-cloner:10.0.3',
    COMPOSE_IMAGE: 'docker/compose:1.11.2',
    KUBE_DEPLOY: 'khulnasoft/cf-deploy-kubernetes:latest',
    FS_OPS_IMAGE: 'khulnasoft/fs-ops:latest',
    TEMPLATE_ENGINE: 'khulnasoft/pikolo:latest',
    PIPELINE_DEBUGGER_IMAGE: 'khulnasoft/cf-debugger:1.1.2',
};

const DEMO_STEP_IMAGE = {
    'docker.io': 'alpine:latest',
    'quay.io': 'quay.io/khulnasoft/alpine:3.11',
    'gcr.io': 'gcr.io/google-containers/alpine-with-bash',
    default: 'quay.io/khulnasoft/alpine:3.11',
};
...
async function getTestPipelineLink(pipelineName, pipeline) {
    const url = _.get(sdk, 'config.context.url', 'https://g.khulnasoft.com');
...
async function createTestPipeline(runtimeName, pipelineName, pipelineCommands, dockerRegistry) {
    ...
    pipeline.spec.steps.test = {
        ...
        title: 'test',
        image: demoStepImage,
        commands: pipelineCommands || ['echo hello Khulnasoft Runner!'],
    };
...
function drawKhulnasoftFiglet() {
    return Promise.race([new Promise((resolve) => {
        figlet('KHULNSOFT', (err, data) => {
            if (err) {
                resolve();
            }
            if (data) {
...
async function getComponent(location = KHULNSOFT_PATH, component, bypassDownload) {
...
async function downloadComponents(location = KHULNSOFT_PATH, componentsArg = []) {
    ...
    for (const component of componentsArray) {
        switch (component) {
            ...
            case 'argocd-agent':
                ...
                break;
            case 'khulnasoft':
                await downloadProvider({ provider: 'khulnasoft', location });
                console.log(colors.green(`Khulnasoft gitops provider downloaded successfully to ${location}`));
                break;
            default:
...
function mergeValuesFromValuesFile(argv, valuesFile, handleError) {
    ...
    if (!_.has(_argv, 'url') && _.has(valuesObj, 'KhulnasoftHost')) {
        _.set(_argv, 'url', valuesObj.KhulnasoftHost);
    }
