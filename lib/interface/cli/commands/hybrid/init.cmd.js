...
const handleError = createErrorHandler(`\nIf you had any issues with the installation please report them at: ${colors.blue('https://github.com/khulnasoft/cli/issues/new')}`);
...
const initCmd = new Command({
    ...
    description: 'Install Khulnasoft Runner solution\'s components on Kubernetes cluster',
    webDocs: {
        category: 'Runner',
        title: 'Init',
        weight: 100,
    ...
    builder: yargs => yargs
        ...
        .option('build-node-selector', {
            describe: 'The Kubernetes node selector "key=value" to be used by the Khulnasoft build resources (default is no node selector)',
        })
        .option('yes', {
        ...
        .option('skip-cluster-integration', {
            describe: 'Do not create a cluster integration in Khulnasoft',
            type: 'boolean',
        })
        .option('set-default-runtime', {
            describe: 'Set this as the default runtime environment for your Khulnasoft account',
            type: 'boolean',
        })
        ...
        .option('install-monitor', {
            describe: 'Install a monitoring component that will help provide valueable data about your cluster to Khulnasoft',
            type: 'boolean',
            default: true,
        })
        ...
        .option('values', {
            describe: 'specify values in a YAML file (see example here: https://github.com/khulnasoft/venona/blob/release-1.0/venonactl/example/values-example.yaml)',
        })
        .option('set-value', {
        ...
        .option('env-vars', {
            describe: 'Addiontal env vars to be used in all Khulnasoft components',
            type: array,
            default: [],
        })
        .option('app-proxy', {
        ...
        .example(
            'khulnasoft runner init --values values.yaml',
            'See values file example here: https://github.com/khulnasoft/venona/blob/release-1.0/venonactl/example/values-example.yaml)',
        ),
    handler: async (argv) => {
        ...
        if (_.get(sdk, 'config.context.isNoAuth') && !token) {
            console.log('Not authenticated as a Khulnasoft account: ');
            console.log('In order to install a Khulnasoft Runner you need to provide ' +
                `an authentication token which can be generated here: ${colors.blue(`${_argv.url}/user/settings`)}` +
                '\nAfter getting the token you may run this command again with the [--token] option or use the \'khulnasoft auth\' command to create an authenticated context.');
            process.exit(1);
        }
        ...
        if (noQuestions) {
            ...
            console.log(colors.green('This installer will guide you through the Khulnasoft Runner installation process'));
            if (!kubeContextName) {
            ...
            if (httpProxy) {
                questions.push({
                    ...
                    name: 'httpProxy',
                    default: httpProxy,
                    message: 'detected HTTP_PROXY, should I use this in all Khulnasoft components?',
                });
            }
            ...
            if (httpsProxy) {
                questions.push({
                    ...
                    name: 'httpsProxy',
                    default: httpsProxy,
                    message: 'detected HTTPS_PROXY, should I use this in all Khulnasoft components?',
            ...
            if (noProxy) {
                questions.push({
                    ...
                    name: 'noProxy',
                    default: noProxy,
                    message: 'detected NO_PROXY, should I use this in all Khulnasoft components?',
            ...
            if (_.isUndefined(shouldMakeDefaultRe)) {
                if (!_.get(sdk, 'config.context.isNoAuth') && await isNewAccount()) {
                    ...
                    let message = 'Set this as the default runtime environment for your Khulnasoft account?';
        ...
        if (token) {
            ...
            console.log(`A Khulnasoft context named '${INSTALLATION_DEFAULTS.CF_CONTEXT_NAME}' was added to your "cfconfig" file.`);
        }
        ...
        installationPlan.addStep({
            ...
            func: async () => {
                ...
                console.log(`A Khulnasoft Runner with the name: ${colors.cyan(agentName)} has been created.`);
        ...
        installationPlan.addStep({
            ...
            func: async () => {
                ...
                _.set(body, 'runtimeScheduler.image', `${dockerRegistry}/khulnasoft/engine:master`);
                _.set(body, 'dockerDaemonScheduler.dindImage', `${dockerRegistry}/khulnasoft/dind:18.09.5-v24-sp`);
        ...
        installationPlan.addStep({
            ...
            condition: async () => {
                ...
                if (isInCluster()) {
                    console.log('Cluster integration was not added , please add it via khulnasoft "create clusters [name]"');
                    return false;
                }
        ...
        function shouldInstallMonitoringFn() {
            ...
            if (isInCluster() || skipClusterIntegration) {
                console.log('Monitor component cannot be installed without cluster integration, you can install it seperately using: "khulnasoft install monitor"');
                return false;
            }
        ...
        installationPlan.addStep({
            ...
            func: async () => {
                ...
                const global = {
                    namespace: kubeNamespace,
                    khulnasoftHost: sdk.config.context.url,
                    agentToken: agent.token,
                    agentId: agent.id,
                    agentName: agent.name,
                    accountId: agent.account,
                    runtimeName: runtimeNameContext,
                    khulnasoftToken: userToken,
                    keys,
                };
        ...
        if (installMonitor) {
            console.log(`Go to ${colors.blue('https://g.khulnasoft.com/kubernetes/services/')} to view your cluster in Khulnasoft dashbaord`);
        }

        console.log(`Link to the new runtime: ${colors.blue(`https://g.khulnasoft.com/account-admin/account-conf/runtime-environments?runtime=${encodeURI(installationPlan.getContext('runtimeName'))}`)}`);
        console.log(`\nDocumentation link: ${colors.blue('https://khulnasoft.com/docs/docs/enterprise/khulnasoft-runner/#khulnasoft-runner-preview-release')}`);
        console.log(`If you had any issues with the installation please report them at: ${colors.blue('https://github.com/khulnasoft/cli/issues/new')}`);
        ...
        if (shouldUseHelm) {
            ...
            console.log('    helm repo add cf-runtime https://chartmuseum.khulnasoft.com/cf-runtime');
            ...
            console.log(`    khulnasoft runner execute-test-pipeline --runtime-name ${installationPlan.getContext('runtimeName')}\n`);
        }
