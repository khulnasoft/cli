...
const setCommand = new Command({
    ...
    builder: (yargs) => {
        return yargs
            ...
            .example('khulnasoft cli-config set', 'Print available property names')
            .example('khulnasoft cli-config set output.pretty false', 'Set "output.pretty" property')
            .example('khulnasoft cli-config set request.maxRetries 10', 'Set "request.maxRetries" property')
            .example('khulnasoft cli-config set request.retryDelay 500', 'Set "request.retryDelay" property');
    },
    handler: async (argv) => {
        if (argv.help) {
            yargs.showHelp();
