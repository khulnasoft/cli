...
const getCommand = new Command({
    ...
    builder: (yargs) => {
        return yargs
            ...
            .example('khulnasoft cli-config get', 'Print available property names')
            .example('khulnasoft cli-config get output', 'Print properties, containing "output" word')
            .example('khulnasoft cli-config get output.pretty', 'Print properties, containing "output.pretty" path');
    },
    handler: async (argv) => {
        if (argv.help) {
            yargs.showHelp();
