...
const command = new Command({
    ...
    builder: (yargs) => {
        ...
        return yargs
            ...
            .example('khulnasoft patch runtime-environments -f ./re.yml', 'Apply changes to a runtime-environment');
    },
    handler: async (argv) => {
        const option = {};
        ...
        if (argv.default) {
            try {
                await sdk.runtimeEnvs.setDefault({ name: option.name });
        ...
        await sdk.runtimeEnvs.update({ name, extend, description }, body);
