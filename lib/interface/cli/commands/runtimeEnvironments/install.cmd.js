...
async function createClusterInPlatform(argv) {
    ...
    try {
        ...
        // create the cluster in khulnasoft if does not exists
        if (createCluster) {
            console.log(`Adding cluster "${colors.cyan(runtimeKubeContextName)}" integration to your Khulnasoft account`);
            try {
                ...
                console.log(`Failed to register cluster on Khulnasoft, cause: ${error.message}`);
            }
        }
    } catch (error) {
...
async function installRuntimeInCluster(argv, runtimeName) {
    ...
    // attach RE to agent in khulnasoft
